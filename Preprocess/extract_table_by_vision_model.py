...
def initialize_chain(
    ...
    model = AzureChatOpenAI(
        ...
        temperature=0.01,
        top_p=0.95,
        max_tokens=5000 # notice the TPM limit and error output might cause by llm
    )
        ...
        for i in range(0, len(image_paths), batch_size):
            ...
            for image_path in batch:
                try:
                    ...
                    ## Append to user content for LLM input (multiple images)
                    # user_content.append({"type": "image_url", "image_url": {"url": image_data_url}})
                    # user_content.append({"type": "text","text": "\n"})

                    # Create messages with "single image" content
                    messages = [
            ...
            try:
                ...
                # logging.info(f"{file_number} finished batch {i+batch_size}/{len(image_paths)}")
...
def main():
    ...
    with open("./PDF_list.txt", 'r') as f:
        for line in f:
            ...
            prefix = file_name[:-4]
            directory = "path to pdf converted images"
    ...
    for doc_num in image_paths:
        ...
        total_results = process_images_in_batches(
            ...
            batch_size=10, # notice the endpoint rate limit
            delay=delay,
